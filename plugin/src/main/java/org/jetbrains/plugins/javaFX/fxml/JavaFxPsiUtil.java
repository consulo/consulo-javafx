...
public class JavaFxPsiUtil {
  ...
  public static PsiClass getControllerClass(final PsiFile containingFile) {
    if (containingFile instanceof XmlFile) {
      ...
      final PsiClass injectedControllerClass = RecursionManager.doPreventingRecursion(containingFile, true, new Computable<PsiClass>() {
        @Override
      ...
      if (fxmlLoader != null) {
        ...
        if (injectControllerMethods.length == 1) {
          ...
          final GlobalSearchScope globalSearchScope = GlobalSearchScope
                      .notScope(GlobalSearchScope.getScopeRestrictedByFileTypes(myContainingFile.getResolveScope(), XmlFileType.INSTANCE));
