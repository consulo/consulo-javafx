...
public class JavaFxExpandAttributeIntention extends PsiElementBaseIntentionAction {
    ...
    public JavaFxExpandAttributeIntention() {
        setText(LocalizeValue.localizeTODO("Expand attribute to tag"));
    }
        ...
        if (element instanceof XmlToken && ((XmlToken) element).getTokenType() == XmlTokenType.XML_NAME) {
            ...
            if (parent instanceof XmlAttribute) {
                ...
                if (descriptor instanceof JavaFxPropertyAttributeDescriptor && !(descriptor instanceof JavaFxDefaultAttributeDescriptor)) {
                    ...
                    PsiClass tagClass = PsiUtil.resolveClassInType(
                        tagType instanceof PsiPrimitiveType ? ((PsiPrimitiveType) tagType).getBoxedType(parent) : tagType
                    );
                    if ((tagClass != null && JavaFxPsiUtil.isAbleToInstantiate(tagClass) == null)
                        || descriptor instanceof JavaFxStaticPropertyAttributeDescriptor) {
                        setText(LocalizeValue.localizeTODO("Expand '" + ((XmlAttribute) parent).getName() + "' to tag"));
                        return true;
                    }
                }
            }
